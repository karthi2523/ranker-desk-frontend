        ...
        <Card className={`border-slate-800 bg-slate-900/50 ${device.isCurrent ? "border-indigo-500/50 bg-indigo-500/5" : ""}`}>
            <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2 gap-4">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                    <div className={`p-2 rounded-full shrink-0 ${device.isCurrent ? "bg-indigo-500/20 text-indigo-400" : "bg-slate-800 text-slate-400"}`}>
                        <Icon className="h-5 w-5" />
                    </div>
                    <div className="min-w-0 flex-1">
                        <CardTitle className="text-base font-medium text-white truncate" title={device.name}>
                            {device.name}
                        </CardTitle>
